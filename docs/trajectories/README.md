...
- **CSS hiding (`display:none` / `visibility:hidden` / the `hidden` attribute) and risky `aria-hidden`** — **now rejected by the gate.** This was previously uncovered by the whole stack, and it mattered more than a single missed gate: Layer B's visibility filter *drops hidden elements*, so hiding an offending control made its violation "resolve" across the pipeline. The guard's snapshot now counts markup-level hiding and rejects any increase, with `aria-hidden` classified **risky** (focusable, contains a control, or carries text — rejected) vs **decorative** (a text-free non-focusable glyph inside a labelled control — accepted, because that is the recommended pattern and our own fixer emits it). Proven directly in [`test/regression-guard.test.ts`](../../test/regression-guard.test.ts) — the two tests that used to *document* this blind spot now assert the rejection. Residual, so this isn't read as "we catch everything": the gate reads markup, not computed style, so hiding via an external stylesheet class would still pass it, and the bare generically-named `<img>` alt case above remains uncovered. Independently, [`test/no-hidden-content.test.ts`](../../test/no-hidden-content.test.ts) measures that our reported numbers contain **zero** hiding artifacts — across all 27 scored pages and all 85 LLM candidates in the sealed prompt set.
