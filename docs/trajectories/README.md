...
- **CSS hiding (`display:none` / `visibility:hidden` / `hidden`) and `aria-hidden`** — **not caught anywhere in the stack**, and we should own that it is worse than a single missed gate: Layer B's visibility filter *drops hidden elements*, so hiding an offending control makes its violation "resolve" across the whole pipeline. This is a genuine hole in "never ship a fix you can't verify." **But it is not a live defect in our reported numbers, and we measured that rather than asserting it:** [`test/no-hidden-content.test.ts`](../../test/no-hidden-content.test.ts) runs the advanced agent across **all 27 scored pages** in replay and asserts no applied fix introduces `display:none`, `visibility:hidden`, the `hidden` attribute, or `aria-hidden` on anything a user needs (a focusable element, an element containing one, or element text); it additionally checks **every LLM fix candidate in the sealed prompt set** (85 fixer responses — a superset of what was applied). Result: **zero** hiding introductions, so no reported true-fix is a hidden-content artifact. The one thing candidates *do* add is `aria-hidden` on decorative glyphs (`<span>▶</span>` inside a labelled button), which is correct practice, not laundering — the test measures the risky class separately from the benign one so the two can't be conflated.
