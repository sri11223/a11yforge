...
2. **Real-world — 206 hidden barriers** (141 excluding one site's repeated ticker finding).
   ...
   usable, in the wild.
3. **Mechanism — harm eliminated, and it replicates.** A fair single-shot baseline ships 6
   regressions + 2 false-fixes; the verify-loop + regression guard ship **zero** (same model,
   same prompt). Measured on **two corpora**, and significant on the larger one.
...
### Two corpora, one conclusion

| Measure | n=27 (sealed, determinism-proven) | n=45 (replication) |
|---|---|---|
...
The 45-page set adds 18 pages generated by a different procedure (`injected-v2`). Every pattern
reproduces. We say plainly what each corpus can carry: at n=27 there are only 5 discordant pairs, and
with c=0 the smallest attainable two-sided exact p is 0.0625 — that corpus **cannot** reach α=0.05 on
this contrast by construction, so we claim no significance there. The replication is larger and
independently generated, but still ours, not an external dataset.
